...
def setup_loki_logging():
    ...
    # Adiciona StreamHandler (Console) como principal e fallback
    ...
    # Verifica se as configurações do Loki estão presentes
    if not settings.LOKI_URL:
        logger.warning("LOKI_URL não configurada. Logs serão enviados apenas para o console.")
        return logger
    ...
    if not any(isinstance(h, (logging_loki.LokiHandler, logging_loki.LokiQueueHandler)) for h in logger.handlers):
        ...
        try:
            ...
            # Usando LokiQueueHandler para envio assíncrono
            ...
            logger.error(f"Erro ao configurar Loki logging: {e}")
